...
func TestProtocol(t *testing.T) {
	...
	t.Run("suscribe single result", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.TODO(), 5*time.Second)
		defer cancel()
	...
	t.Run("suscribe without init", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.TODO(), 5*time.Second)
		defer cancel()
	...
	t.Run("suscribe id re-use", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.TODO(), 5*time.Second)
		defer cancel()
...
func TestProtocol_InitFunc(t *testing.T) {
	...
	t.Run("accept connection if InitFunc returns no error", func(t *testing.T) {
		...
		h := testserver.New()
		h.AddTransport(&Protocol{
			InitFunc: func(r *http.Request, op ObjectPayload) (context.Context, ObjectPayload, error) {
				return nil, nil, nil
			},
		})
...
func TestProtocol_InitTimeout(t *testing.T) {
	...
	var ce websocket.CloseError
	require.ErrorAs(t, err, &ce)
	require.Equal(t, 4408, int(ce.Code))
}
